...
## 回测与评估(大目标·阶段九,Python 侧)

> 一句话:纯 pandas 手写回测——信号→shift 成仓位→仓位×涨跌→累乘资金曲线;指标看总收益/最大回撤/交易数。回测是理解机制的脚手架,绝非赚钱证明。

1. **躲未来函数:`signal.shift(1)` = 今天仓位用昨天信号**
   信号是收盘才算出来的,不可能当天按它交易。不 shift = 偷看未来,回测虚高。这是回测头号坑。

2. **pandas 三件套**:列与列逻辑用 `& | ~` 且每个比较加括号(`&&`/`and` 都不行);`(条件).sum()` = 数满足条件的行(True 当 1);`.cumprod()` 累乘资金曲线、`.cummax()` 历史最高、回撤 = `equity/cummax - 1` 取 min。

3. **三记重锤(亲测数字)**:四线多头策略 +146% vs **buy&hold +3498%**(死拿赢 24 倍)——频繁择时常输躺平;最大回撤 **-57%**——总收益正也曾腰斩,回撤才考验人性;363 次交易的手续费没算(磨掉几十个点)+ 拿茅台回测本身就是幸存者偏差。**回测数字永远打骨折看。**

---

## 多轮多空辩论(共享 transcript 的多轮 Agent 对话)

> 一句话:一份不断增长的辩论记录(transcript),两个对立 Agent 轮流"读全文→反驳对方最新观点→追加自己的话",循环 N 轮后交决策 Agent。比线性流水线高一维:Agent 之间真正"对上话"了。

1. **结构核心 = 共享可增长的上下文**
   `transcript += 发言` 后,下一个发言者就能读到并针对性反驳。除"第一个发言者的第一轮"(只能开场陈述,无对手可驳,符合真实辩论)外,每次发言都看得到对方最新论点。实测第 2 轮双方都在逐条回击对方上一轮。

2. **发言顺序是设计参数:最后发言有优势**
   每轮最后发言的一方(这里是看空),其反驳最新且无人再回应,会更影响决策 Agent。实测同一只股票三次结论:看多→观望→看空(这次看空收尾,决策翻向看空)。想公平可轮流收尾。**结构塑造结论 + LLM 自带随机性 = 又一条"盯机制不盯收益"。**
